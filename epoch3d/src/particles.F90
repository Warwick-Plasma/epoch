...
!!$       IF (part_species(ipart) == 0) CYCLE
       ...
       part_x  = Cur%Part_pos(1) - x_start_local
       part_y  = Cur%Part_pos(2) - y_start_local
       part_z  = Cur%Part_pos(3) - z_start_local
       part_px = Cur%Part_P(1)
       part_py = Cur%Part_P(2)
       part_pz = Cur%Part_P(3)
       part_species = Cur%Part_Species
...
!!$       ENDIF
       ...
       sum(cell_x1-1,cell_y1-1,cell_z1-1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1-1,part_species)+sum_local*gmx*gmy*gmz
       sum(cell_x1,cell_y1-1,cell_z1-1,part_species)=sum(cell_x1,cell_y1-1,cell_z1-1,part_species)+sum_local*g0x*gmy*gmz
       sum(cell_x1+1,cell_y1-1,cell_z1-1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1-1,part_species)+sum_local*gpx*gmy*gmz

       sum(cell_x1-1,cell_y1,cell_z1-1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1-1,part_species)+sum_local*gmx*g0y*gmz
       sum(cell_x1,cell_y1,cell_z1-1,part_species)=sum(cell_x1,cell_y1-1,cell_z1-1,part_species)+sum_local*g0x*g0y*gmz
       sum(cell_x1+1,cell_y1,cell_z1-1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1-1,part_species)+sum_local*gpx*g0y*gmz

       sum(cell_x1-1,cell_y1+1,cell_z1-1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1-1,part_species)+sum_local*gmx*gpy*gmz
       sum(cell_x1,cell_y1+1,cell_z1-1,part_species)=sum(cell_x1,cell_y1-1,cell_z1-1,part_species)+sum_local*g0x*gpy*gmz
       sum(cell_x1+1,cell_y1+1,cell_z1-1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1-1,part_species)+sum_local*gpx*gpy*gmz

       sum(cell_x1-1,cell_y1-1,cell_z1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1,part_species)+sum_local*gmx*gmy*g0z
       sum(cell_x1,cell_y1-1,cell_z1,part_species)=sum(cell_x1,cell_y1-1,cell_z1,part_species)+sum_local*g0x*gmy*g0z
       sum(cell_x1+1,cell_y1-1,cell_z1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1,part_species)+sum_local*gpx*gmy*g0z

       sum(cell_x1-1,cell_y1,cell_z1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1,part_species)+sum_local*gmx*g0y*g0z
       sum(cell_x1,cell_y1,cell_z1,part_species)=sum(cell_x1,cell_y1-1,cell_z1,part_species)+sum_local*g0x*g0y*g0z
       sum(cell_x1+1,cell_y1,cell_z1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1,part_species)+sum_local*gpx*g0y*g0z

       sum(cell_x1-1,cell_y1+1,cell_z1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1,part_species)+sum_local*gmx*gpy*g0z
       sum(cell_x1,cell_y1+1,cell_z1,part_species)=sum(cell_x1,cell_y1-1,cell_z1,part_species)+sum_local*g0x*gpy*g0z
       sum(cell_x1+1,cell_y1+1,cell_z1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1,part_species)+sum_local*gpx*gpy*g0z

       sum(cell_x1-1,cell_y1-1,cell_z1+1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1+1,part_species)+sum_local*gmx*gmy*gpz
       sum(cell_x1,cell_y1-1,cell_z1+1,part_species)=sum(cell_x1,cell_y1-1,cell_z1+1,part_species)+sum_local*g0x*gmy*gpz
       sum(cell_x1+1,cell_y1-1,cell_z1+1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1+1,part_species)+sum_local*gpx*gmy*gpz

       sum(cell_x1-1,cell_y1,cell_z1+1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1+1,part_species)+sum_local*gmx*g0y*gpz
       sum(cell_x1,cell_y1,cell_z1+1,part_species)=sum(cell_x1,cell_y1-1,cell_z1+1,part_species)+sum_local*g0x*g0y*gpz
       sum(cell_x1+1,cell_y1,cell_z1+1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1+1,part_species)+sum_local*gpx*g0y*gpz

       sum(cell_x1-1,cell_y1+1,cell_z1+1,part_species)=sum(cell_x1-1,cell_y1-1,cell_z1+1,part_species)+sum_local*gmx*gpy*gpz
       sum(cell_x1,cell_y1+1,cell_z1+1,part_species)=sum(cell_x1,cell_y1-1,cell_z1+1,part_species)+sum_local*g0x*gpy*gpz
       sum(cell_x1+1,cell_y1+1,cell_z1+1,part_species)=sum(cell_x1+1,cell_y1-1,cell_z1+1,part_species)+sum_local*gpx*gpy*gpz
...
!!$       IF (cell_x1 .LT. 0) WRITE(rank+10,*),"Particle low X init"
!!$       IF (cell_x2 .LT. 0) WRITE(rank+10,*),"Particle low X half"
!!$
!!$       IF (cell_y1 .LT. 0) WRITE(rank+10,*),"Particle low Y init"
!!$       IF (cell_y2 .LT. 0) WRITE(rank+10,*),"Particle low Y half"
!!$
!!$       IF (cell_z1 .LT. 0) WRITE(rank+10,*),"Particle low Y init"
!!$       IF (cell_z2 .LT. 0) WRITE(rank+10,*),"Particle low Y half"
       ...
       Cur%Part_pos(1) = part_x + x_start_local
       Cur%Part_pos(2) = part_y + y_start_local
       Cur%Part_pos(3) = part_z + z_start_local
...
!!$       IF (ABS(cell_x3-cell_x1-1) .GT. 2) THEN
!!$          PRINT *,"ERROR ",cell_x1,cell_y1,cell_z1,rank
!!$          PRINT *,"ERROR2 ",cell_x2,cell_y2,cell_z2
!!$       ENDIF
       ...
       DO iz=zmin,zmax
          DO iy=ymin,ymax
             DO ix=xmin,xmax
                ...
                jxh(ix,iy,iz)=jxh(ix-1,iy,iz) - Part_q * wx * 1.0_num/dt * weight/(dy*dz)
                jyh(ix,iy,iz)=jyh(ix,iy-1,iz) - Part_q * wy * 1.0_num/dt * weight/(dx*dz)
                jzh(ix,iy,iz)=jzh(ix,iy,iz-1) - Part_q * wz * 1.0_num/dt * weight/(dx*dy)
...
!!$       PRINT *,Cycling
    ...
    DO iSpecies=1,nSpecies
       CALL Full_Swap(sum(:,:,:,iSpecies))
       CALL Full_Swap(ct(:,:,:,iSpecies))
    ENDDO
          ...
          DO iy=1,ny
             DO ix=1,nx
                IF (ct(ix,iy,iz,ipart) .GT. 0) THEN
                   mean=sum(ix,iy,iz,ipart)/ct(ix,iy,iz,ipart)
                ELSE
                   mean=0.0_num
                ENDIF
                temperature(ix,iy,iz,ipart)=mean
             ENDDO
          ENDDO
       ENDDO
    ENDDO
    ...
    DO iz=-1,1
       DO iy=-1,1
          DO ix=-1,1
             Sizes(ix,iy,iz)=3
             IF (ix == 0) THEN
                Sizes(ix,iy,iz) = Sizes(ix,iy,iz) * nxp
                xStart(ix,iy,iz) = 0
                xEnd(ix,iy,iz) = nxp
             ELSE IF (ix==1) THEN
             ...
             ENDIF
             IF (iy == 0) THEN
                Sizes(ix,iy,iz) = Sizes(ix,iy,iz) * nyp
                yStart(ix,iy,iz) = 0
                yEnd(ix,iy,iz) = nyp
             ELSE IF (iy==1) THEN
             ...
             ENDIF
             IF (iz == 0) THEN
                Sizes(ix,iy,iz) = Sizes(ix,iy,iz) * nzp
                zStart(ix,iy,iz) = 0
                zEnd(ix,iy,iz) = nzp
             ELSE IF (iz==1) THEN
...
!!$             ENDIF
    ...
    CALL DoField_MPI(Array)

  END SUBROUTINE Full_Swap
