...
CONTAINS
  ...
  SUBROUTINE after_control
    ...
    x_start_local=x_start+length_x*REAL(coordinates(3),num)/REAL(nprocx,num)
    x_end_local=x_start+length_x*REAL(coordinates(3)+1,num)/REAL(nprocx,num)
    length_x_local=x_end_local-x_start_local


    y_start_local=y_start+length_y*REAL(coordinates(2),num)/REAL(nprocy,num)
    y_end_local=y_start+length_y*REAL(coordinates(2)+1,num)/REAL(nprocy,num)
    length_y_local=y_end_local-y_start_local


    z_start_local=z_start+length_z*REAL(coordinates(1),num)/REAL(nprocz,num)
    z_end_local=z_start+length_z*REAL(coordinates(1)+1,num)/REAL(nprocz,num)
    length_z_local=z_end_local-z_start_local
    ...
    DO ix=1,nx
       x(ix)=x(ix-1)+dx
    ENDDO
    y(0)=y_start_local-dy
    DO iy=1,ny
       y(iy)=y(iy-1)+dy
    ENDDO
    z(0)=z_start_local-dz
    DO iz=1,nz
       z(iz)=z(iz-1)+dz
    ENDDO
  ...
  SUBROUTINE open_files
    ...
    IF (rank == 0) THEN
       WRITE(file2, '(a,"/pic3d.dat")') TRIM(data_dir)
...
!!$    REAL(num), DIMENSION(3) :: extents,stagger
...
!!$          ENDIF
!!$
!!$          IF (nd .NE. DIMENSION_3D .AND. nd .NE. DIMENSION_IRRELEVANT ) THEN
...
!!$                IF (rank .EQ. 0) PRINT *,"Number of gridpoints does not match, gridpoints in file is",dims(1),"x",dims(2),"x",dims(3)
!!$                CALL MPI_ABORT(comm,errcode)
!!$             ENDIF
!!$             IF (StrCmp(Name(1:2),"Ex")) CALL cfd_Get_3D_Cartesian_Variable_All(Ex(1:nx,1:ny,1:nz))
!!$             IF (StrCmp(Name(1:2),"Ey")) CALL cfd_Get_3D_Cartesian_Variable_All(Ey(1:nx,1:ny,1:nz))
!!$             IF (StrCmp(Name(1:2),"Ez")) CALL cfd_Get_3D_Cartesian_Variable_All(Ez(1:nx,1:ny,1:nz))
!!$
!!$             IF (StrCmp(Name(1:2),"Bx")) CALL cfd_Get_3D_Cartesian_Variable_All(Bx(1:nx,1:ny,1:nz))
!!$             IF (StrCmp(Name(1:2),"By")) CALL cfd_Get_3D_Cartesian_Variable_All(By(1:nx,1:ny,1:nz))
!!$             IF (StrCmp(Name(1:2),"Bz")) CALL cfd_Get_3D_Cartesian_Variable_All(Bz(1:nx,1:ny,1:nz))
!!$
!!$          CASE(VAR_PARTICLE)
