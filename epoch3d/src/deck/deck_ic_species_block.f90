...
CONTAINS
  ...
  FUNCTION handle_ic_species_deck(species_id,element,value)
    ...
    IF (str_cmp(element,"temp")) THEN
       CALL evaluate_string_in_space(value,initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,1),(/-1,nx+2/),(/-1,ny+2/),(/-1,nz+2/),handle_ic_species_deck)
       initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,2)=initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,1)
       initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,3)=initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,1)
       RETURN
    ENDIF
    ...
    IF (str_cmp(element,"temp_y")) THEN
       CALL evaluate_string_in_space(value,initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,2),(/-1,nx+2/),(/-1,ny+2/),(/-1,nz+2/),handle_ic_species_deck)
       RETURN
    ENDIF

    IF (str_cmp(element,"temp_z")) THEN
       CALL evaluate_string_in_space(value,initial_conditions(species_id)%temp(-1:nx+2,-1:ny+2,-1:nz+2,3),(/-1,nx+2/),(/-1,ny+2/),(/-1,nz+2/),handle_ic_species_deck)
       RETURN
    ENDIF
