...
CONTAINS
  ...
  FUNCTION handle_ic_external_species_deck(species_id, element, value)
    ...
    IF (str_cmp(element, "temp")) THEN
      CALL load_data_file(value, initial_conditions(species_id)%temp(:, :, :, 1), handle_ic_external_species_deck)
    ...
    IF (str_cmp(element, "temp_y")) THEN
      CALL load_data_file(value, initial_conditions(species_id)%temp(:, :, :, 2), handle_ic_external_species_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "temp_z")) THEN
      CALL load_data_file(value, initial_conditions(species_id)%temp(:, :, :, 3), handle_ic_external_species_deck)
      RETURN
    ENDIF
