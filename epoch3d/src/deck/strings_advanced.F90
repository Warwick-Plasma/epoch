...
!!$    PRINT *,TRIM(StrIn)," A ",TRIM(StrIn(1:pos-1))," B ",TRIM(StrIn(pos+1:strlen))
  ...
  SUBROUTINE EvaluateStringInSpace(StrIn,DataOut,xrange,yrange,zrange,ERR)
    ...
    DO iz=zrange(1),zrange(2)
       DO iy=yrange(1),yrange(2)
          DO ix=xrange(1),xrange(2)
             DataOut(ix-xrange(1)+1,iy-yrange(1)+1,iz-yrange(1)+1) = EvaluateAtPoint(output,ix,iy,iz,ERR)
          ENDDO
       ENDDO
    ENDDO
