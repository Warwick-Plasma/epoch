...
MODULE bethe_heitler
  ...
  REAL(num), PRIVATE, PARAMETER :: two_mc2 = 2.0_num * mc2
...
CONTAINS
...
#ifdef HYBRID
  SUBROUTINE hy_bethe_heitler_update_depth

    ! Cycles through all photon species and updates the Bethe-Heitler optical
    ! depths (labelled bremsstrahlung for photons). This routine is designed for
    ! making pairs in the hybrid mode, where we look at solids instead of ion
    ! number densities
  ...
  SUBROUTINE generate_pair(photon, z_int, ielectron, ipositron, iphoton)
    ...
    momentum_1 = SQRT(energy_1**2 - mc2**2)/c
    momentum_2 = SQRT(energy_2**2 - mc2**2)/c
  ...
  FUNCTION energy_split(photon, z_int)
    ...
    split_0 = mc2 / photon%particle_energy
  ...
  SUBROUTINE sample_theta(energy_1, energy_2, theta_1, theta_2)
    ...
    theta_1 = u_samp * mc2 / energy_1
    theta_2 = u_samp * mc2 / energy_2
