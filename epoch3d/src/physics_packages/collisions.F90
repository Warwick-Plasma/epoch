          ...
          DO ix = 1, nx
            ...
            IF (e_user_factor > 0.0_num) THEN
              CALL inter_species_collisions(ejected_e, ionising_e, &
                  ...
                  e_temp(ix,iy,iz), jtemp(ix,iy,iz), e_log_lambda(ix,iy,iz), &
                  e_user_factor)
            ENDIF
            ...
            CALL append_partlist( &
                species_list(jspecies)%secondary_list(ix,iy,iz), ejected_e)
          ENDDO ! ix
          ENDDO ! iy
          ENDDO ! iz
          ...
          DO ix = 1, nx
            ...
            IF (e_user_factor > 0.0_num) THEN
              CALL inter_species_collisions(ejected_e, ionising_e, &
                  ...
                  itemp(ix,iy,iz), e_temp(ix,iy,iz), e_log_lambda(ix,iy,iz), &
                  e_user_factor)
            ENDIF
            ! Scatter non-ionising impact electrons off of remaining unionised
            ! targets provided target has charge
            ...
            CALL append_partlist( &
                species_list(ispecies)%secondary_list(ix,iy,iz), ejected_e)
          ENDDO ! ix
          ENDDO ! iy
          ENDDO ! iz
        ELSE
...
!    coll_freq = MAX(coll_freq, vrabs / (jdens**(1.0_num / 3.0_num)))
  ...
  PURE FUNCTION calc_coulomb_log(temp1, temp2, ekbar1, dens1, dens2, q1, q2, m1)
    ...
    REAL(num), DIMENSION(-2:,-2:,-2:), INTENT(IN) :: temp1, temp2, ekbar1
    ...
    DO j = -2, ny+3
    DO i = -2, nx+3
      local_temp1 = MAX(temp1(i,j,k), 100.0_num)
      ...
      ELSE
        kDeSq = dens1(i,j,k) * q1**2 / (epsilon0 * q0 * local_temp1)
        kDiSq = dens2(i,j,k) * q2**2 / (epsilon0 * q0 * local_temp2)
