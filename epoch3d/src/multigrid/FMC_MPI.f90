...
!!$       ENDIF 
  ...
  SUBROUTINE Restrict(hires,lores,grid_hi,grid_lo)
    ...
    DO ix = 1 ,nx_l
       DO iy = 1, ny_l
          lores(ix,iy)=(4.0_num*hires(ix*2-1,iy*2-1) + 2.0_num * (hires(ix*2,iy*2-1)+hires(ix*2-2,iy*2-1)&
               +hires(ix*2-1,iy*2)+hires(ix*2-1,iy*2-2)) +  (hires(ix*2,iy*2) &
               + hires(ix*2,iy*2-2) + hires(ix*2-2,iy*2) + hires(ix*2-2,iy*2-2)))/16.0_num
       ENDDO
    ENDDO
  ...
  SUBROUTINE V_Cycle(start_grid)
    ...
    DO WHILE (.NOT. (grids(start_grid)%Converged) .AND. count .LE. eta)
       CALL DoGrid(grids(start_grid))
       count=count+1
    ENDDO

  END SUBROUTINE V_Cycle

  RECURSIVE SUBROUTINE DoGrid(grid)
  ...
  SUBROUTINE Allocate_Grids
    ...
    DO igrid=1,ngrids
       CALL Allocate_SingleGrid(grids(igrid),igrid-1,dx,dy)
    ENDDO
  ...
  END SUBROUTINE Allocate_SingleGrid
