...
CONTAINS
  ...
  SUBROUTINE output_routines(i)   ! i = step index
    ...
    IF (print_arrays) THEN
      ...
      IF (IAND(dumpmask(1), code) .NE. 0) CALL cfd_write_nd_particle_grid_with_iterator_all("Particles", "Part_Grid", iterate_particles, 3, npart_local, npart_dump_global, npart_per_it, c_particle_cartesian, subtype_particle_var)
      ...
      IF (IAND(dumpmask(2), code) .NE. 0) THEN
        ...
        ELSE
          CALL cfd_write_3d_cartesian_grid("Grid", "Grid", x_offset_global(1:nx_global), y_offset_global(1:ny_global) , z_offset_global(1:nz_global), 0)
          CALL cfd_write_3d_cartesian_grid("Grid_Full", "Grid", x_global(1:nx_global), y_global(1:ny_global) , z_global(1:nz_global), 0)
        ENDIF
      ENDIF
      ...
      IF (IAND(dumpmask(9), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Ex", "Electric Field", dims, stagger, "Grid" , "Grid", ex(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(10), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Ey", "Electric Field", dims, stagger, "Grid" , "Grid", ey(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(11), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Ez", "Electric Field", dims, stagger, "Grid" , "Grid", ez(1:nx, 1:ny, 1:nz), subtype_field)

      IF (IAND(dumpmask(12), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Bx", "Magnetic Field", dims, stagger, "Grid" , "Grid", bx(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(13), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("By", "Magnetic Field", dims, stagger, "Grid" , "Grid", by(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(14), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Bz", "Magnetic Field", dims, stagger, "Grid" , "Grid", bz(1:nx, 1:ny, 1:nz), subtype_field)

      IF (IAND(dumpmask(15), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Jx", "Current", dims, stagger, "Grid", "Grid" , jx(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(16), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Jy", "Current", dims, stagger, "Grid", "Grid" , jy(1:nx, 1:ny, 1:nz), subtype_field)
      IF (IAND(dumpmask(17), code) .NE. 0) CALL cfd_write_3d_cartesian_variable_parallel("Jz", "Current", dims, stagger, "Grid", "Grid" , jz(1:nx, 1:ny, 1:nz), subtype_field)
...
!!$    CALL MPI_FILE_SET_VIEW(filehandle, current_displacement, MPI_INTEGER, subtype_particle_int, $         "native", MPI_INFO_NULL, cfd_errcode)
