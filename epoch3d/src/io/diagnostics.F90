  ...
  SUBROUTINE build_species_subset
    ...
    DO i = 1, n_species
      ...
      DO WHILE (ASSOCIATED(current))
        ...
        IF (subset_list(l)%use_z_min) THEN
          IF (current%part_pos(3) < subset_list(l)%z_min) use_particle = .FALSE.
        END IF

        IF (subset_list(l)%use_z_max) THEN
          IF (current%part_pos(3) > subset_list(l)%z_max) use_particle = .FALSE.
        END IF
