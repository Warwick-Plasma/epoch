...
CONTAINS
  ...
  SUBROUTINE cfd_write_nd_particle_grid_all(name, class, particles, npart_global, particle_coord_type, particle_type)
    ...
    md_length = meshtype_header_offset + 1 * soi + 1 * soi8  + ndim * 2 * num ! 1 INT, 1 INT8, 2REAL per Dim
    ...
    CALL MPI_FILE_SET_VIEW(cfd_filehandle, current_displacement, mpireal, particle_type, "native", MPI_INFO_NULL, cfd_errcode)
    ! Write the real data
  ...
  SUBROUTINE cfd_write_nd_particle_grid_with_iterator_all(name, class, iterator, ndims, npart_local, npart_global, npart_per_iteration, particle_coord_type, particle_type)
    ...
    md_length = meshtype_header_offset + 1 * soi + 1 * soi8  + ndims * 2 * num ! 1 INT, 1 INT8, 2REAL per Dim
    ...
    CALL MPI_FILE_SET_VIEW(cfd_filehandle, offset_for_min_max, mpireal, mpireal, "native", MPI_INFO_NULL, cfd_errcode)
    IF (cfd_rank == default_rank) THEN
      CALL MPI_FILE_WRITE(cfd_filehandle, min_max, ndims*2, mpireal, cfd_status, cfd_errcode)
    ENDIF
    ...
    DO idim = 1, ndims
      ...
      DO
        ...
        ELSE
          min_max(idim, 1) = MIN(min_max(idim, 1), MINVAL(DATA(1:npart_this_cycle)))
          min_max(idim, 2) = MAX(min_max(idim, 2), MAXVAL(DATA(1:npart_this_cycle)))
        ENDIF
        start = .FALSE.
    ...
    CALL MPI_FILE_SET_VIEW(cfd_filehandle, offset_for_min_max, mpireal, mpireal, "native", MPI_INFO_NULL, cfd_errcode)
    DO idim = 1, ndims
      CALL MPI_ALLREDUCE(min_max(idim, 1), mn, 1, mpireal, MPI_MIN, cfd_comm, cfd_errcode)
      CALL MPI_ALLREDUCE(min_max(idim, 2), mx, 1, mpireal, MPI_MAX, cfd_comm, cfd_errcode)
      IF (cfd_rank == default_rank) THEN
        CALL MPI_FILE_WRITE(cfd_filehandle, mn, 1, mpireal, cfd_status, cfd_errcode)
        CALL MPI_FILE_WRITE(cfd_filehandle, mx, 1, mpireal, cfd_status, cfd_errcode)
      ENDIF
  ...
  ! Serial operation, so no need to specify nx, ny
  ! -------------------------------------------------------------------------------------------------------------------
  SUBROUTINE cfd_write_nd_particle_variable_all(name, class, particles, npart_global, mesh_name, mesh_class, particle_type)
  ...
  SUBROUTINE cfd_write_nd_particle_variable_with_iterator_all(name, class, iterator, npart_global, npart_per_iteration, mesh_name, mesh_class, particle_type)
