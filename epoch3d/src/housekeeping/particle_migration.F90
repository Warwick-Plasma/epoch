! Copyright (C) 2013-2015 Keith Bennett <K.Bennett@warwick.ac.uk>
...
CONTAINS
  ...
  SUBROUTINE update_fluid_energy(fluid_list)
    ...
    ALLOCATE(tmp(-ng+1:nx+ng, -ng+1:ny+ng, -ng+1:nz+ng))
...
#include "particle_to_grid.inc"
      ...
      DO iy = sf_min, sf_max
      DO ix = sf_min, sf_max
        local_te = local_te + gx(ix) * gy(iy) * gz(iz) &
            * species_list(from_list)%migrate%fluid_energy(cell_x+ix, &
            cell_y+iy, cell_z+iz)
        local_ne = local_ne + gx(ix) * gy(iy) * gz(iz) &
  ...
  SUBROUTINE initialise_migration
    ...
    DO ispecies = 1, n_species
      IF (species_list(ispecies)%migrate%fluid) THEN
        ALLOCATE(species_list(ispecies)%migrate%fluid_energy(-ng+1:nx+ng, &
            -ng+1:ny+ng, -ng+1:nz+ng))
        CALL calc_temperature(species_list(ispecies)%migrate%fluid_energy, &
            ispecies)
        ALLOCATE(species_list(ispecies)%migrate%fluid_density(-ng+1:nx+ng, &
            -ng+1:ny+ng, -ng+1:nz+ng))
