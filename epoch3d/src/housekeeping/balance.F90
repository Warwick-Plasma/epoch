...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    IF (cpml_boundaries) THEN
      ...
      ALLOCATE(cpml_e_psiyx(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psiyx = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_e_psizx, temp)
      DEALLOCATE(cpml_e_psizx)
      ALLOCATE(cpml_e_psizx(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psizx = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psiyx, temp)
      DEALLOCATE(cpml_b_psiyx)
      ALLOCATE(cpml_b_psiyx(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psiyx = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psizx, temp)
      DEALLOCATE(cpml_b_psizx)
      ALLOCATE(cpml_b_psizx(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psizx = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_e_psixy, temp)
      DEALLOCATE(cpml_e_psixy)
      ALLOCATE(cpml_e_psixy(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psixy = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_e_psizy, temp)
      DEALLOCATE(cpml_e_psizy)
      ALLOCATE(cpml_e_psizy(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psizy = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psixy, temp)
      DEALLOCATE(cpml_b_psixy)
      ALLOCATE(cpml_b_psixy(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psixy = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psizy, temp)
      DEALLOCATE(cpml_b_psizy)
      ALLOCATE(cpml_b_psizy(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psizy = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_e_psixz, temp)
      DEALLOCATE(cpml_e_psixz)
      ALLOCATE(cpml_e_psixz(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psixz = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_e_psiyz, temp)
      DEALLOCATE(cpml_e_psiyz)
      ALLOCATE(cpml_e_psiyz(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_e_psiyz = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psixz, temp)
      DEALLOCATE(cpml_b_psixz)
      ALLOCATE(cpml_b_psixz(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psixz = temp

      temp = 0.0_num
      CALL redistribute_field(new_domain, cpml_b_psiyz, temp)
      DEALLOCATE(cpml_b_psiyz)
      ALLOCATE(cpml_b_psiyz(-2:nx_new+3, -2:ny_new+3, -2:nz_new+3))
      cpml_b_psiyz = temp
