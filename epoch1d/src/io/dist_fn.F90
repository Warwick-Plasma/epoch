  ...
  SUBROUTINE general_3d_dist_fn(name,direction,range,resolution,species,restrictions,use_restrictions)
    ...
    DO WHILE(ASSOCIATED(current))
       ...
       IF (use_this) THEN
          ...
          ENDDO
          IF (use_this) data(cell(1),cell(2),cell(3))=data(cell(1),cell(2),cell(3))+current%weight * real_space_area
       ENDIF
       current=>current%next
    ENDDO
  ...
  SUBROUTINE general_2d_dist_fn(name,direction,range,resolution,species,restrictions,use_restrictions)
    ...
    DO WHILE(ASSOCIATED(current))
       ...
       IF (use_this) THEN
          ...
          IF (use_this)data(cell(1),cell(2))=data(cell(1),cell(2))+current%weight*real_space_area
       ENDIF
       current=>current%next
    ENDDO
