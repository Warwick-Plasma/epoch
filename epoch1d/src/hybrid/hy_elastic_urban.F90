...
CONTAINS
  ...
  SUBROUTINE set_Z_cache
    ...
    ! Allocate arrays
    ALLOCATE(urb_d1(1-ng:nx+ng))
    ALLOCATE(urb_d2(1-ng:nx+ng))
    ALLOCATE(urb_d3(1-ng:nx+ng))
    ALLOCATE(urb_d4(1-ng:nx+ng))
    ALLOCATE(urb_Zeff(1-ng:nx+ng))
    ALLOCATE(urb_iZ23(1-ng:nx+ng))
    ALLOCATE(urb_theta1(1-ng:nx+ng))
    ALLOCATE(urb_theta2(1-ng:nx+ng))
    ALLOCATE(urb_rad_len(1-ng:nx+ng))

    ! Allocate temporary arrays
    ALLOCATE(Z16(1-ng:nx+ng))
    ...
    DO ix = 1-ng,nx+ng
      ...
      DO isol = 1,solid_count
        ! Effective Z
        urb_Zeff(ix) = urb_Zeff(ix) &
        + solid_array(isol)%ion_density(ix) &
        * solid_array(isol)%Z
        sum_ni = sum_ni + solid_array(isol)%ion_density(ix)

        ! Radiation length
        IF (solid_array(isol)%ion_density(ix) > ni_max) THEN
          urb_rad_len(ix) = solid_array(isol)%rad_len
          ni_max = solid_array(isol)%ion_density(ix)
        END IF
      END DO
      urb_Zeff(ix) = urb_Zeff(ix)/sum_ni
    END DO
  ...
  FUNCTION compute_lambda1(KE)
    ...
    IF (KE < KE_10MeV) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_iZ23, urb_iZ23)
      ...
      DO isol = 1, solid_count
        ...
        CALL hy_grid_centred_var_at_particle(part_x, part_ni, &
  ...
  FUNCTION compute_theta0(KE, step_size)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_rad, urb_rad_len)
    ...
    IF (species_list(ispecies)%species_type == c_species_id_positron) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_Zeff, urb_Zeff)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_th1, urb_theta1)
    CALL hy_grid_centred_var_at_particle(part_x, part_th2, urb_theta2)
  ...
  FUNCTION sample_cos_theta(step_size, small_step, lambda1, theta0)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, d1, urb_d1)
    CALL hy_grid_centred_var_at_particle(part_x, d2, urb_d2)
    CALL hy_grid_centred_var_at_particle(part_x, d3, urb_d3)
    CALL hy_grid_centred_var_at_particle(part_x, d4, urb_d4)
