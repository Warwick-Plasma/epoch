MODULE deck_control_block
  ...
  CHARACTER(len=EntryLength),DIMENSION(ControlBlockElements) :: ControlBlockName = (/"nx","npart",&
...
CONTAINS
  ...
  FUNCTION CheckControlBlock()
    ...
    IF (IAND(ictype,IC_RESTART)  .EQ. 0) ControlBlockDone(12)=.TRUE.

    !The neutral background is still beta, so hide it if people don't want it
    ControlBlockDone(13)=.TRUE.
