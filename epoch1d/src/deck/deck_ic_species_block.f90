...
CONTAINS
  ...
  FUNCTION HandleICSpeciesDeck(Species_ID,Element,Value)
    ...
    IF (StrCmp(Element,"rho") .OR. StrCmp(Element,"number_density")) THEN
       CALL EvaluateStringInSpace(Value,InitialConditions(Species_ID)%rho(-2:nx+3),(/-2,nx+3/),HandleICSpeciesDeck)
       RETURN
    ENDIF
    ...
    IF (StrCmp(Element,"temp")) THEN
       CALL EvaluateStringInSpace(Value,InitialConditions(Species_ID)%temp(-1:nx+2,1),(/-1,nx+2/),HandleICSpeciesDeck)
    ...
    IF (StrCmp(Element,"temp_y")) THEN
       CALL EvaluateStringInSpace(Value,InitialConditions(Species_ID)%temp(-1:nx+2,2),(/-1,nx+2/),HandleICSpeciesDeck)
       RETURN
    ENDIF

    IF (StrCmp(Element,"temp_z")) THEN
       CALL EvaluateStringInSpace(Value,InitialConditions(Species_ID)%temp(-1:nx+2,3),(/-1,nx+2/),HandleICSpeciesDeck)
       RETURN
    ENDIF
