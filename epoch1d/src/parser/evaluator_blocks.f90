...
CONTAINS
  ...
  SUBROUTINE DoOperator(opcode,ix,err)
  ...
  SUBROUTINE DoConstant(opcode,ix,err)
    ...
    IF (opcode .GE. CONSTANT_CUSTOM_LOWBOUND) THEN
       ...
       Result = CustomConstant(opcode,ix,err)    
  ...
  SUBROUTINE DoFunctions(opcode,ix,err)
    ...
    IF (opcode .EQ. FUNC_RHO) THEN
       CALL GetValues(1,Values)
       CALL PushOnEval(InitialConditions(NINT(Values(1)))%rho(ix))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_TEMPX) THEN
       CALL GetValues(1,Values)
       CALL PushOnEval(InitialConditions(NINT(Values(1)))%temp(ix,1))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_TEMPY) THEN
       CALL GetValues(1,Values)
       CALL PushOnEval(InitialConditions(NINT(Values(1)))%temp(ix,2))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_TEMPZ) THEN
       CALL GetValues(1,Values)
       CALL PushOnEval(InitialConditions(NINT(Values(1)))%temp(ix,3))
       RETURN
    ENDIF
    ...
    IF (opcode .EQ. FUNC_EX) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(Ex(NINT(Values(1))))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_EY) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(Ey(NINT(Values(1))))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_EZ) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(Ez(NINT(Values(1))))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_BX) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(BX(NINT(Values(1))))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_BY) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(By(NINT(Values(1))))
       RETURN
    ENDIF
    IF (opcode .EQ. FUNC_BZ) THEN
       CALL GetValues(2,Values)
       CALL PushOnEval(Bz(NINT(Values(1))))
       RETURN
    ENDIF
    ...
    Result = CustomFunction(opcode,ix,err)
