...
CONTAINS
  ...
  SUBROUTINE Read_Deck(filename,FirstCall)
    ...
    IF (.NOT. code_setup) THEN
       StatusFileName="./deck.status"
    ELSE
       StatusFileName="./IC.status"
    ENDIF
#endif
...
!!$    ENDIF
    ...
    IF (rank .EQ. 0) THEN
       ...
       syscmd=" "
       WRITE(syscmd,"('cp 'a' 'a'/.')"),TRIM(filename),TRIM(data_dir)
       CALL System(TRIM(syscmd))

       IF (FirstCall) THEN
          PRINT *,"Moving deck.status with filename ",filename
          IF (.NOT. code_setup) THEN
          ...
          ELSE
             syscmd=" "
             WRITE(syscmd,"('mv IC.status 'a'/.')"),TRIM(data_dir)
          ENDIF
          CALL System(TRIM(syscmd))
       ENDIF
    ENDIF
  ...
  SUBROUTINE HandleDeckElement(Element,Value,Errcode_deck)
    ...
    PRINT *,"DECK ERRCODE ", ERRCODE_DECK
