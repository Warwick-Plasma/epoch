    ...
    DO ispecies = 1, n_species
      ...
      DO WHILE(ASSOCIATED(current))
        ...
        DO ix = sf_min, sf_max
          ...
          number_at_part = number_at_part &
              + dx * gx(ix) * number_density(cell_x1+ix)
        ENDDO
        ...
        IF (rand .LT. ion_prob) THEN
          ...
          !    / (e_part**2 * epsilon0 * dx))
          !DO ix = -1, 1
          !  ex(cell_x2+ix) = erat * hx(ix) * ex(cell_x2+ix)
          !  ey(cell_x1+ix) = erat * gx(ix) * ey(cell_x1+ix)
          !  ez(cell_x1+ix) = erat * gx(ix) * ez(cell_x1+ix)
          !ENDDO
