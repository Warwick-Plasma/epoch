...
CONTAINS
  ...
  SUBROUTINE run_background_collisions
    ...
    DO i_fast = 1, n_species
      ...
      DO WHILE(ASSOCIATED(fast))
        ...
        cell_x = FLOOR((fast%part_pos - x_grid_min_local) / dx) + 1
#else
        cell_x = FLOOR((fast%part_pos - x_grid_min_local) / dx + 1.5_num)
#endif

        ! Calculate background-averaged Coulomb logarithm in this cell
        IF (coulomb_log_auto) THEN
          cou_log_12 = back_cou_log(cell_x)
        ELSE
        ...
        DO i_back = 1, back_count
          ...
          n2 = back_ni_all(id2array_all(spec2), cell_x)
          m2 = species_list(spec2)%mass
          q2 = species_list(spec2)%charge
  ...
  SUBROUTINE update_background_variables
    ...
    ! which includes:
    !   back_ni_all(i, ix): Number density for background species i at cell ix
    !   back_cou_log(ix):  The Coulomb logarithm in cell ix
    !
    ...
    INTEGER :: ispecies, i_back, ix, spec2
    REAL(num), ALLOCATABLE :: num_dens(:), temp(:), fast_num_dens(:)
    ...
    REAL(num), ALLOCATABLE :: debye_length(:), b0(:), delta_x(:), r_min(:)
    ...
    ALLOCATE(back_ni_all(back_count_all,1-ng:nx+ng))
    ALLOCATE(back_cou_log(1-ng:nx+ng))
    ALLOCATE(num_dens(1-ng:nx+ng))
    IF (coulomb_log_auto) THEN
      ALLOCATE(temp(1-ng:nx+ng))
      ALLOCATE(debye_length(1-ng:nx+ng))
      ALLOCATE(b0(1-ng:nx+ng))
      ALLOCATE(delta_x(1-ng:nx+ng))
      ALLOCATE(r_min(1-ng:nx+ng))
      ALLOCATE(fast_num_dens(1-ng:nx+ng))
      ALLOCATE(current_ekbar(1-ng:nx+ng))
      ALLOCATE(fast_ekbar(1-ng:nx+ng))
      ALLOCATE(fast_p(1-ng:nx+ng))
      ALLOCATE(q1q2(1-ng:nx+ng))
      ALLOCATE(pair_num_dens(1-ng:nx+ng))
      ALLOCATE(pair_num_dens_tot(1-ng:nx+ng))
    ...
    DO ispecies = 1, n_species
      ...
      IF (coulomb_log_auto) THEN
        ...
        DO ix = 1-ng,nx+ng
          ni = MAX(num_dens(ix), 1.0_num)
          ti = MAX(temp(ix), 1.0_num)
    ...
    IF (coulomb_log_auto) THEN
      ...
      DO ix = 1-ng,nx+ng
        ...
        fast_ekbar(ix) = MAX(fast_ekbar(ix), q0) &
            / MAX(fast_num_dens(ix), 1.0_num)
        ! <momentum> of all fast particles in a cell
        fast_p(ix) = MAX(fast_p(ix), q0 / c) / MAX(fast_num_dens(ix), 1.0_num)
        ! Heisenburg uncertainty length for fast particles in each cell
        delta_x(ix) = h_planck / (2.0_num * fast_p(ix))
        ! <q1*q2> averaged over all fast/background pairs
        q1q2(ix) = q1q2(ix) / MAX(pair_num_dens_tot(ix), 1.0_num)
      END DO
      ...
      IF (use_cold_correction) THEN
        ...
        ! Calculate r_min
        DO ix = 1-ng,nx+ng
          n2_tot = MAX(r_min(ix), 1.0_num)
          r_min(ix) = (4.0_num * pi * n2_tot / 3.0_num)**(-1.0_num / 3.0_num)
        END DO
      END IF

      ! Coulomb logarithm
      DO ix = 1-ng,nx+ng
        bmax = MAX(debye_length(ix), r_min(ix))
        bmin = MAX(delta_x(ix), b0(ix))
        back_cou_log(ix) = MAX(1.0_num, LOG(bmax / bmin))
      END DO
