...
CONTAINS
  ...
  SUBROUTINE particle_collisions
    ...
    ALLOCATE(idens(-ng+1:nx+ng))
    ALLOCATE(jdens(-ng+1:nx+ng))
    ALLOCATE(itemp(-ng+1:nx+ng))
    ALLOCATE(jtemp(-ng+1:nx+ng))
    ALLOCATE(log_lambda(-ng+1:nx+ng))
    ALLOCATE(meanx(-ng+1:nx+ng))
    ALLOCATE(meany(-ng+1:nx+ng))
    ALLOCATE(meanz(-ng+1:nx+ng))
    ALLOCATE(part_count(-ng+1:nx+ng))
    ALLOCATE(iekbar(-ng+1:nx+ng))
  ...
  SUBROUTINE collisional_ionisation
    ...
    ALLOCATE(idens(-ng+1:nx+ng))
    ALLOCATE(jdens(-ng+1:nx+ng))
    ALLOCATE(e_dens(-ng+1:nx+ng))
    ALLOCATE(itemp(-ng+1:nx+ng))
    ALLOCATE(jtemp(-ng+1:nx+ng))
    ALLOCATE(e_temp(-ng+1:nx+ng))
    ALLOCATE(log_lambda(-ng+1:nx+ng))
    ALLOCATE(e_log_lambda(-ng+1:nx+ng))
    ALLOCATE(meanx(-ng+1:nx+ng))
    ALLOCATE(meany(-ng+1:nx+ng))
    ALLOCATE(meanz(-ng+1:nx+ng))
    ALLOCATE(part_count(-ng+1:nx+ng))
    ALLOCATE(iekbar(-ng+1:nx+ng))
    ALLOCATE(e_ekbar(-ng+1:nx+ng))
...
!    coll_freq = MAX(coll_freq, vrabs / (jdens**(1.0_num / 3.0_num)))
  ...
  PURE FUNCTION calc_coulomb_log(ekbar1, temp2, dens1, dens2, q1, q2, m1)
    ...
    REAL(num), DIMENSION(-ng+1:), INTENT(IN) :: ekbar1, temp2
    REAL(num), DIMENSION(-ng+1:), INTENT(IN) :: dens1, dens2
    REAL(num), INTENT(IN) :: q1, q2, m1
    REAL(num), DIMENSION(-ng+1:nx+ng) :: calc_coulomb_log
...
#include "particle_to_grid.inc"
        ...
        DO ix = sf_min, sf_max
          gf = gx(ix)
          data_array(cell_x+ix) = data_array(cell_x+ix) + gf * wdata
          part_count(cell_x+ix) = part_count(cell_x+ix) + gf * part_w
        ENDDO ! ix
