...
CONTAINS
  ...
  SUBROUTINE bremsstrahlung_update_optical_depth
    ...
    INTEGER :: ispecies, iZ, Z_temp, iArray
    TYPE(particle), POINTER :: current
    ...
    IF (use_plasma_screening) THEN
      ...
      DO iArray = 1-ng, nx+ng
        IF (grid_num_density_electron(iArray) < 1.0e-10_num) THEN
          grid_root_temp_over_num(iArray) = 0.0_num
        ELSE IF (grid_temperature_electron(iArray) < 1.0e-10_num) THEN
          grid_root_temp_over_num(iArray) = 0.0_num
        ELSE
          grid_root_temp_over_num(iArray) = &
          SQRT(grid_temperature_electron(iArray) / &
              grid_num_density_electron(iArray))
        END IF
      END DO
