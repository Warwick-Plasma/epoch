...
CONTAINS
  ...
  SUBROUTINE cfd_Get_nD_Particle_Grid_Parallel_With_Iterator(ndims,npart_local,npart_per_it,SubType,Iterator)
    ...
    Start=.TRUE.
    ALLOCATE(Data(1:ndims,1:npart_per_it))
    npart_remain=npart_local
    npart_this_it=MIN(npart_remain,npart_per_it)
    DO WHILE (npart_this_it .GT. 0)
       npart_this_it=MIN(npart_remain,npart_per_it)
       CALL MPI_FILE_READ(cfd_filehandle, Data,npart_this_it * ndims,mpireal,cfd_status,cfd_errcode)
