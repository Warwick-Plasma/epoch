...
CONTAINS
  ...
  SUBROUTINE setup_data_averaging()
    ...
    DO ioutput = 1, num_vars_to_dump
      IF (IAND(dumpmask(ioutput), c_io_averaged) .NE. 0) THEN
        ...
        ALLOCATE(averaged_data(ioutput)%array(-2:nx+3,n_species_local))
  ...
  SUBROUTINE setup_species
    ...
    DO ispecies = 1, n_species
      ...
      particle_species(ispecies)%density = 0.0_num
      particle_species(ispecies)%temperature = 0.0_num
  ...
  SUBROUTINE restart_data(snap)
    ...
    CHARACTER(LEN=c_max_string_length) :: str1
    CHARACTER(LEN=c_max_string_length) :: name, class, mesh_name, mesh_class
    REAL(KIND=8) :: time_d
    REAL(num), DIMENSION(4) :: extents, stagger
