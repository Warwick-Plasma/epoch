...
! temperature.
  SUBROUTINE twotemp_loadv
    ...
    IF(ABS(species%initial_conditions%density_back) > c_tiny .AND. twot) THEN
    ...
    DO WHILE(ipart < partlist%count)
      ...
      CALL set_single_particle_temperature(&
          ...
          species, current, species_list(ispecies)%initial_conditions%drift(:,1), tfac)
      CALL set_single_particle_temperature(&
          species_list(ispecies)%initial_conditions%temp(:,2), c_dir_y, &
          species, current, species_list(ispecies)%initial_conditions%drift(:,2), tfac)
      CALL set_single_particle_temperature(&
          species_list(ispecies)%initial_conditions%temp(:,3), c_dir_z, &
          species, current, species_list(ispecies)%initial_conditions%drift(:,3), tfac)
  ...
  SUBROUTINE set_single_particle_temperature(temperature, direction, part_species, current, &
      drift, tfac)
