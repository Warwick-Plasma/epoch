...
CONTAINS
  ...
  FUNCTION create_current_field_subtype()
    ...
    create_current_field_subtype = create_field_subtype(nx, ny, &
        cell_x_start(coordinates(2)+1), cell_y_start(coordinates(1)+1))
  ...
  FUNCTION create_field_subtype(nx_local, ny_local, cell_start_x_local, &
    ...
    DO iy = 0, ny_local-1
      ipoint = ipoint+1
      starts(ipoint) = &
          (cell_start_y_local+iy-1) * nx_global + cell_start_x_local -1
    ENDDO

    CALL MPI_TYPE_INDEXED(ny_local, lengths, starts, mpireal, &
