...
CONTAINS
  ...
  SUBROUTINE smooth_array(array)
      ...
      DO iy = 1, ny
        DO icyclex = -sf_order, sf_order
          DO icycley = -sf_order, sf_order
            wk_array(ix, iy) = wk_array(ix, iy) + array(ix+icyclex, iy+icycley) * weight_fn(icyclex) * weight_fn(icycley)
          ENDDO
        ENDDO
      ENDDO
    ENDDO
    ...
    DO ix = 1, nx
      DO iy = 1, ny
        wk_array(ix, iy) = (array(ix, iy) + array(ix-1, iy)*0.25_num + array(ix+1, iy)*0.25_num + array(ix, iy-1) *0.25_num + array(ix, iy+1)*0.25_num)*0.5_num
      ENDDO
    ENDDO
#endif
