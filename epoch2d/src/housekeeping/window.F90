...
MODULE window
  ...
  ! In order to check for steps that do I/O, we call 'io_test' from the
  ! 'diagnostics' module. This would create a circular module dependency between
  ! 'diagnostics' and 'window'. A submodule is currently used to solve the
  ! circular dependency. This is not nice, but works.
  ! Modules 'setup' and 'diagnostics' both require a single variable from this
  ! module: 'window_shift_fraction'. If this variable were to be more 'global',
  ! the submodule could be removed. An alternative is a separate module for only
  ! this single variable that 'window', 'diagnostics' and 'setup' then use.
  INTERFACE
    MODULE SUBROUTINE moving_window(future_step, force_dump)
