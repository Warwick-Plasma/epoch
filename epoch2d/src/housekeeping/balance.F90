...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    IF (use_hybrid) THEN
     ...
     resistivity_model = temp_int
      ...
      IF (use_hybrid_scatter .AND. elastic_scatter_model == c_hy_urban) THEN
        ...
        DEALLOCATE(urb_d1)
        ALLOCATE(urb_d1(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_d1 = temp

        CALL remap_field(urb_d2, temp)
        DEALLOCATE(urb_d2)
        ALLOCATE(urb_d2(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_d2 = temp

        CALL remap_field(urb_d3, temp)
        DEALLOCATE(urb_d3)
        ALLOCATE(urb_d3(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_d3 = temp

        CALL remap_field(urb_d4, temp)
        DEALLOCATE(urb_d4)
        ALLOCATE(urb_d4(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_d4 = temp

        CALL remap_field(urb_iZ23, temp)
        DEALLOCATE(urb_iZ23)
        ALLOCATE(urb_iZ23(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_iZ23 = temp

        CALL remap_field(urb_theta1, temp)
        DEALLOCATE(urb_theta1)
        ALLOCATE(urb_theta1(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_theta1 = temp

        CALL remap_field(urb_theta2, temp)
        DEALLOCATE(urb_theta2)
        ALLOCATE(urb_theta2(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_theta2 = temp

        CALL remap_field(urb_Zeff, temp)
        DEALLOCATE(urb_Zeff)
        ALLOCATE(urb_Zeff(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_Zeff = temp

        CALL remap_field(urb_rad_len, temp)
        DEALLOCATE(urb_rad_len)
        ALLOCATE(urb_rad_len(1-ng:nx_new+ng, 1-ng:ny_new+ng))
        urb_rad_len = temp
      END IF
    ...
    DO WHILE(ASSOCIATED(hy_current))
      ...
      ALLOCATE(hy_current%profile(1-ng:ny_new+ng))
    ...
    DO WHILE(ASSOCIATED(hy_current))
      ...
      DEALLOCATE(hy_current%profile)
      ALLOCATE(hy_current%profile(1-ng:nx_new+ng))
      hy_current%profile = temp_slice
