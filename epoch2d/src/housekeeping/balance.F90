...
CONTAINS
  ...
  SUBROUTINE redistribute_fields(new_domain)
    ...
    DO WHILE(ASSOCIATED(current))
      temp1d = 0.0_num
      CALL redistribute_field_1d(new_domain(2, 1), new_domain(2, 2), cell_y_start(coordinates(1)+1) , cell_y_end(coordinates(1)+1), ny_global, current%profile, temp1d, c_dir_x)
      DEALLOCATE(current%profile)
      ALLOCATE(current%profile(-2:ny_new+3))
      current%profile = temp1d
      temp1d = 0.0_num
      CALL redistribute_field_1d(new_domain(2, 1), new_domain(2, 2), cell_y_start(coordinates(1)+1) , cell_y_end(coordinates(1)+1), ny_global, current%phase, temp1d, c_dir_x)
    ...
    DO WHILE(ASSOCIATED(current))
      temp1d = 0.0_num
      CALL redistribute_field_1d(new_domain(1, 1), new_domain(1, 2), cell_x_start(coordinates(2)+1) , cell_x_end(coordinates(2)+1), nx_global, current%profile, temp1d, c_dir_y)
      DEALLOCATE(current%profile)
      ALLOCATE(current%profile(-2:nx_new+3))
      current%profile = temp1d
      temp1d = 0.0_num
      CALL redistribute_field_1d(new_domain(1, 1), new_domain(1, 2), cell_x_start(coordinates(2)+1) , cell_x_end(coordinates(2)+1), nx_global, current%phase, temp1d, c_dir_y)
      DEALLOCATE(current%phase)
  ...
  SUBROUTINE redistribute_field(domain, field, new_field)
    ...
    CALL MPI_FILE_READ_ALL(fh, new_field(1:nx_new, 1:ny_new), nx_new*ny_new, mpireal, status, errcode)
  ...
  FUNCTION get_particle_processor(a_particle)
    ...
    DO iproc = 0, nprocx-1
      IF (a_particle%part_pos(1) .GE. x_starts(iproc) - dx/2.0_num .AND. a_particle%part_pos(1) .LE. x_ends(iproc) + dx/2.0_num) THEN
        coords(2) = iproc
        EXIT
      ENDIF
    ENDDO
    ...
    DO iproc = 0, nprocy-1
      IF (a_particle%part_pos(2) .GE. y_starts(iproc) -dy/2.0_num .AND. a_particle%part_pos(2) .LE. y_ends(iproc) + dy/2.0_num) THEN
        coords(1) = iproc
        EXIT
      ENDIF
    ENDDO
