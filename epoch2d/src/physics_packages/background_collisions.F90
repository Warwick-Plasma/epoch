...
CONTAINS
    ...
    DO i_fast = 1, n_species
      ...
      DO WHILE(ASSOCIATED(fast))
        ...
        IF (coulomb_log_auto) THEN
          cou_log_12 = back_cou_log(cell_x, cell_y)
        ELSE
        ...
        DO i_back = 1, back_count
          ...
          n2 = back_ni_all(id2array_all(spec2), cell_x, cell_y)
          m2 = species_list(spec2)%mass
          q2 = species_list(spec2)%charge
  ...
  SUBROUTINE update_background_variables
    ...
    ! which includes:
    !   back_ni_all(i, ix, iy): Number density for background species i at cell
    !                           (ix,iy)
    !   back_cou_log(ix, iy):  The Coulomb logarithm in cell (ix,iy)
    !
    ...
    INTEGER :: ispecies, i_back, ix, iy, spec2
    ...
    ALLOCATE(back_ni_all(back_count_all,1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(back_cou_log(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(num_dens(1-ng:nx+ng,1-ng:ny+ng))
    IF (coulomb_log_auto) THEN
      ALLOCATE(temp(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(debye_length(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(b0(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(delta_x(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(r_min(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(fast_num_dens(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(current_ekbar(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(fast_ekbar(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(fast_p(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(q1q2(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(pair_num_dens(1-ng:nx+ng,1-ng:ny+ng))
      ALLOCATE(pair_num_dens_tot(1-ng:nx+ng,1-ng:ny+ng))
    ...
    DO ispecies = 1, n_species
      ...
      IF (coulomb_log_auto) THEN
        ...
        DO iy = 1-ng,ny+ng
          DO ix = 1-ng,nx+ng
            ni = MAX(num_dens(ix, iy), 1.0_num)
            ti = MAX(temp(ix, iy), 1.0_num)
    ...
    IF (coulomb_log_auto) THEN
      ...
      DO iy = 1-ng,ny+ng
        DO ix = 1-ng,nx+ng
          ...
          fast_ekbar(ix, iy) = MAX(fast_ekbar(ix, iy), q0) &
              / MAX(fast_num_dens(ix, iy), 1.0_num)
          ! <momentum> of all fast particles in a cell
          fast_p(ix, iy) = MAX(fast_p(ix, iy), q0 / c) &
              / MAX(fast_num_dens(ix, iy), 1.0_num)
          ! Heisenburg uncertainty length for fast particles in each cell
          delta_x(ix, iy) = h_planck / (2.0_num * fast_p(ix, iy))
          ! <q1*q2> averaged over all fast/background pairs
          q1q2(ix,iy) = q1q2(ix,iy) / MAX(pair_num_dens_tot(ix,iy), 1.0_num)
        END DO
      END DO
      ...
      IF (use_cold_correction) THEN
        ...
        DO iy = 1-ng,ny+ng
          DO ix = 1-ng,nx+ng
            n2_tot = MAX(r_min(ix,iy), 1.0_num)
            r_min(ix, iy) = &
      ...
      DO iy = 1-ng,ny+ng
        DO ix = 1-ng,nx+ng
          bmax = MAX(debye_length(ix,iy), r_min(ix,iy))
          bmin = MAX(delta_x(ix,iy), b0(ix,iy))
          back_cou_log(ix, iy) = MAX(1.0_num, LOG(bmax / bmin))
        END DO
      END DO
