...
CONTAINS
  ...
  SUBROUTINE hy_k_alpha_update_optical_depth
    ...
    REAL(num) :: part_x, part_y, part_e, part_ke, part_ni, part_v
      ...
      DO ispecies = 1, n_species
        ...
        IF (part_type == c_species_id_electron) THEN
          ...
          DO WHILE(ASSOCIATED(current))
            ...
            CALL grid_centred_var_at_particle(part_x, part_y, part_z_st, &
                ion_charge)
            z_st = NINT(part_z_st)
            ...
            CALL grid_centred_var_at_particle(part_x, part_y, part_te, hy_te)
  ...
  SUBROUTINE grid_centred_var_at_particle(part_x, part_y, part_var, grid_var)
