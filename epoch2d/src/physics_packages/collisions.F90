...
!    coll_freq = MAX(coll_freq, vrabs / (jdens**(1.0_num / 3.0_num)))
  ...
  PURE FUNCTION calc_coulomb_log(temp1, temp2, ekbar1, dens1, dens2, q1, q2, m1)
    ...
    REAL(num), DIMENSION(-2:,-2:), INTENT(IN) :: temp1, temp2, ekbar1
    ...
    DO j = -2, ny+3
    DO i = -2, nx+3
      local_temp1 = MAX(temp1(i,j), 100.0_num)
      ...
      ELSE
        kDeSq = dens1(i,j) * q1**2 / (epsilon0 * q0 * local_temp1)
        kDiSq = dens2(i,j) * q2**2 / (epsilon0 * q0 * local_temp2)
