...
CONTAINS
  ...
  SUBROUTINE collisional_ionisation
    ...
    DO ispecies = 1, n_species
      ...
      IF (ABS(species_list(ispecies)%charge) .LE. c_tiny) THEN
      ...
      DO jspecies = ispecies, n_species
        ...
        IF (ABS(species_list(jspecies)%charge) .LE. c_tiny) THEN
...
!    gamma_en = 1 + en_after / (mass * c**2)
  ...
  PURE FUNCTION calc_coulomb_log(temp, dens, q1, q2)
    ...
    REAL(num), DIMENSION(-2:,-2:), INTENT(IN) :: temp, dens
    ...
    DO i = -2, nx+3
      ...
      ELSE
        ...
        IF (ratio .LE. efac) THEN
          calc_coulomb_log(i,j) = 1.0_num
        ELSE
          calc_coulomb_log(i,j) = lfac + 0.5_num * LOG(ratio)
        ENDIF
      ENDIF
    ENDDO
    ENDDO
