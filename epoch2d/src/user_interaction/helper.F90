...
CONTAINS
  ...
  SUBROUTINE allocate_ic
    ...
    DO ispecies = 1, n_species
      ALLOCATE(initial_conditions(ispecies)%density(-2:nx+3,-2:ny+3))
      ...
      initial_conditions(ispecies)%density_max = -1.0_num
    ENDDO
  ...
  SUBROUTINE non_uniform_load_particles(density, species_list, density_min, &
    ...
    DO ix = 1, nx
      DO iy = 1, ny
        ipart = 0
        npart_per_cell = INT(density(ix, iy) / density_average &
    ...
    species_list%weight = density_total_global * dx * dy / npart_this_species
