...
#include "particle_to_grid.inc"
    ...
    DO ix = sf_min, sf_max
      ...
      j = cell_y + iy
      gf = gx(ix) * gy(iy)
      temp_local = temp_local + gf * temperature(i,j)
      drift_local = drift_local + gf * drift(i,j)
    END DO
    END DO
  ...
  SUBROUTINE density_local(current, density, dens_local)
    ...
    TYPE(particle), POINTER, INTENT(IN) :: current
    REAL(num), DIMENSION(1-ng:,1-ng:), INTENT(IN) :: density
...
#include "particle_to_grid.inc"
    ...
    DO ix = sf_min, sf_max
      i = cell_x + ix
      j = cell_y + iy
      dens_local = dens_local + gx(ix) * gy(iy) * density(i,j)
    END DO
    END DO
