...
CONTAINS
  ...
  FUNCTION handle_species_deck(element, value)
    ...
    IF (str_cmp(element, "drift_x")) THEN
      ...
      ELSE
        CALL evaluate_string_in_space(value, &
            initial_conditions(species_id)%drift(-2:nx+3,-2:ny+3, 1), &
            (/-2,nx+3/), (/-2,ny+3/), handle_species_deck)
      ENDIF
      RETURN
    ENDIF
    ...
    IF (str_cmp(element, "drift_y")) THEN
      ...
      ELSE
        CALL evaluate_string_in_space(value, &
            initial_conditions(species_id)%drift(-2:nx+3,-2:ny+3, 2), &
            (/-2,nx+3/), (/-2,ny+3/), handle_species_deck)
      ENDIF
      RETURN
    ENDIF
    ...
    IF (str_cmp(element, "drift_z")) THEN
      ...
      ELSE
        CALL evaluate_string_in_space(value, &
            initial_conditions(species_id)%drift(-2:nx+3,-2:ny+3, 3), &
            (/-2,nx+3/), (/-2,ny+3/), handle_species_deck)
      ENDIF
      RETURN
    ENDIF
