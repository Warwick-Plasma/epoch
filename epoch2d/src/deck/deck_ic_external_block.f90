...
CONTAINS
  ...
  FUNCTION handle_ic_external_species_deck(species_id, element, value)
    ...
    IF (str_cmp(element, "rho")) THEN
      CALL load_single_array_from_data_file(value, &
  ...
  FUNCTION handle_ic_external_fields_deck(element, value)
    ...
    IF (str_cmp(element, "ex")) THEN
      CALL load_single_array_from_data_file(value, ex, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "ey")) THEN
      CALL load_single_array_from_data_file(value, ey, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "ez")) THEN
      CALL load_single_array_from_data_file(value, ez, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "bx")) THEN
      CALL load_single_array_from_data_file(value, bx, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "by")) THEN
      CALL load_single_array_from_data_file(value, by, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF

    IF (str_cmp(element, "bz")) THEN
      CALL load_single_array_from_data_file(value, bz, offset, &
          handle_ic_external_fields_deck)
      RETURN
    ENDIF
