...
MODULE hy_elastic_urban
  ...
  REAL(num), PRIVATE :: part_x, part_y
...
CONTAINS
  ...
  SUBROUTINE set_Z_cache
    ...
    ! Allocate arrays
    ALLOCATE(urb_d1(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_d2(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_d3(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_d4(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_Zeff(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_iZ23(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_theta1(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_theta2(1-ng:nx+ng,1-ng:ny+ng))
    ALLOCATE(urb_rad_len(1-ng:nx+ng,1-ng:ny+ng))

    ! Allocate temporary arrays
    ALLOCATE(Z16(1-ng:nx+ng,1-ng:ny+ng))
    ...
    DO iy = 1-ng,ny+ng
      DO ix = 1-ng,nx+ng
        ...
        DO isol = 1,solid_count
          ! Effective Z
          urb_Zeff(ix,iy) = urb_Zeff(ix,iy) &
          + solid_array(isol)%ion_density(ix,iy) &
          * solid_array(isol)%Z
          sum_ni = sum_ni + solid_array(isol)%ion_density(ix,iy)

          ! Radiation length
          IF (solid_array(isol)%ion_density(ix,iy) > ni_max) THEN
            urb_rad_len(ix,iy) = solid_array(isol)%rad_len
            ni_max = solid_array(isol)%ion_density(ix,iy)
          END IF
        END DO
        urb_Zeff(ix,iy) = urb_Zeff(ix,iy)/sum_ni
      END DO
    END DO
  ...
  FUNCTION compute_lambda1(KE)
    ...
    IF (KE < KE_10MeV) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_y, part_iZ23, &
          urb_iZ23)
    ...
    ELSE
        sigma = 0
        DO isol = 1,solid_count
          CALL hy_grid_centred_var_at_particle(part_x, part_y, &
  ...
  FUNCTION compute_theta0(KE, step_size)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_rad, &
        urb_rad_len)
    ...
    IF (species_list(ispecies)%species_type == c_species_id_positron) THEN
      ...
      CALL hy_grid_centred_var_at_particle(part_x, part_y, part_Zeff, &
          urb_Zeff)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_th1, &
        urb_theta1)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, part_th2, &
        urb_theta2)
  ...
  FUNCTION sample_cos_theta(step_size, small_step, lambda1, theta0)
    ...
    CALL hy_grid_centred_var_at_particle(part_x, part_y, d1, urb_d1)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, d2, urb_d2)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, d3, urb_d3)
    CALL hy_grid_centred_var_at_particle(part_x, part_y, d4, urb_d4)
