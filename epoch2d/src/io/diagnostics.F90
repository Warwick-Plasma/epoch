  ...
  SUBROUTINE output_routines(i)   ! i = step index
    ...
    IF (IAND(dumpmask(c_dump_part_grid), code) .NE. 0) &
        CALL cfd_write_nd_particle_grid_with_iterator_all(cfd_handle, &
            "Particles", "Part_Grid", iterate_particles, c_dimension_2d, &
    ...
    IF (IAND(dumpmask(c_dump_grid), code) .NE. 0) THEN
      IF (.NOT. use_offset_grid) THEN
        CALL cfd_write_2d_cartesian_grid(cfd_handle, "Grid", "Grid", &
            x_global(1:nx_global), y_global(1:ny_global), 0)
      ELSE
        CALL cfd_write_2d_cartesian_grid(cfd_handle, "Grid", "Grid", &
            x_offset_global(1:nx_global), y_offset_global(1:ny_global), 0)
        CALL cfd_write_2d_cartesian_grid(cfd_handle, "Grid_Full", "Grid", &
            x_global(1:nx_global), y_global(1:ny_global), 0)
      ENDIF
    ENDIF
...
#ifdef FIELD_DEBUG
    data = rank
    CALL cfd_write_2d_cartesian_variable_parallel(cfd_handle, "Rank", &
